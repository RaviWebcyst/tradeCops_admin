                ...
                <table class="table border text-nowrap customize-table mb-0 align-middle">
                  ...
                  <tbody v-if="users.length > 0">
                    <tr v-for="(user,i) in users">
                      ...
                      <td>{{ user.email }}</td>
                      <td>{{ user.phone }}</td>
                      <td> <span class="badge  fw-semibold fs-2" :class="user.kyc_status == 'Pending'?'bg-light-warning text-warning':(user.kyc_status=='Completed'?'bg-light-success text-success':'bg-light-danger text-danger')">{{ user.kyc_status }}</span></td>
